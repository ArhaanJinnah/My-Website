window.onload = function() {
    ...
    const startMinute = 20;  // 20 minutes past 3 PM
    const endHour = 12;  // 12:00 PM the next day

    // Check if the current time is between 3:20 PM today and 12:00 PM the next day
    if (
        (hours > startHour || (hours === startHour && minutes >= startMinute)) && // After 3:20 PM
        (hours < endHour || (hours === endHour && minutes === 0)) // Before 12:00 PM the next day
    ) {
        document.body.style.display = "block";  // Show the website
    } else {
        document.body.style.display = "none";  // Hide the website
        alert("This website is only available between 3:20 PM and 12:00 PM Pakistan Time.");
    }
};
